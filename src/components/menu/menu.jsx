...
const Menu = ({GetActiveThemeId, NewHeader}) => {
    ...
    useEffect(()=>{
        axios.get('http://localhost:3001/Theme?_expand=color').then(({data}) => {
            setTaskingState(data)
        })
    },[NewThemeAdd,NewHeader])
    ...
    useEffect(()=>{
        if(DeleteState)
        {
            axios.delete('http://localhost:3001/Theme/' + DeleteState.id).then(() => {    
                SetActiveTask(null);
                
        })
            axios.get("http://localhost:3001/Task").then(({data}) => {
                data.forEach(Task => {
                    if(Task.ThemeId === DeleteState.id){
                        axios.delete("http://localhost:3001/Task/" + Task.id).catch(error => console.error(error))
                    }
                })
            })
        setDeleteState(null)
    ...
    return(
        <div className = "Menu">
            <div className="Menu__wrapper" onClick = {() => {setAllActive(true); SetActiveTask(null) }}>
